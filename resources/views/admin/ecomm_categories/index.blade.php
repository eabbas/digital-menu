...
@section('content')
    <div class="flex flex flex-row items-center gap-5 mb-10">
        <label for="">فیلتر بر اساس فروشگاه:</label>
        <div class="items-center flex flex-cols justify-between">
            <select class="outline-none px-4 py-3 bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-2xl shadow-sm focus:shadow-lg focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition-all duration-300"
                    onchange="getEcommCategories(this,'filter')" class="bg-grey-500" name="ecomm_id" id="ecomm_id">
    ...
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            ...
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center">همه دسته ها</h2>

            <form class="flex flex-col gap-5" action="{{ route('ecomm_category.deleteAll') }}" method="post">
                @csrf
                ...
                <div
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]" id="ecomCategories">
                        @php
                            $i = 1;
                        @endphp
                        @foreach (Auth::user()->ecomm_categories as $category)
                            <div
                                ...
                                <div
                                    ...
                                    <div class="w-20 lg:w-full">
                                        <a href="{{ route('ecomm.single', [$category->id]) }}"
                                           class="block w-20 lg:w-full">
                                ...
                                <div
                                    ...
                                    <a href="{{ route('ecomm.single', [$category->id]) }}"
                                       class="block w-20 lg:w-full">{{ $category->title }}</a>
                                </div>
                                <div class="col-span-4">
                                    <div
                                            class="w-[320px] lg:w-full grid grid-cols-3 divide-x divide-[#f1f1f4] items-center">

                                        <ul class="text-sm mt-1 rounded-sm p-1 grid grid-cols-3">
                                            <li class="flex justify-center">
                                                <a href="{{ route('ecomm_category.show', [$category->id]) }}"
                                            ...
                                            @if($category->title != 'بدون دسته بندی')
                                                <li class="flex justify-center">
                                                    <a href="{{ route('ecomm_category.edit_ecomm_categories', [$category->id]) }}"
                                                ...
                                                <li class="flex justify-center">
                                                    <a href="{{ route('ecomm_category.delete', [$category->id]) }}"
                                        ...
                                        <div
                                            ...
                                            <a href="{{ route('ecomm_product.category_product', [$category->id]) }}"
                                               class="text-sky-700">مشاهده محصولات</a>
                                        </div>
                                        @if($category->title != "بدون دسته بندی")
                                            <div
                                                    class="p-1 lg:p-3 text-xs text-center lg:text-sm h-full flex items-center justify-center font-medium">
                                                <a href="{{ route('ecomm_product.singleCreate', [$category->id]) }}"
                                                   class="text-sky-700">محصول جدید</a>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                ...
                success: function (categories) {
                    ...
                    categories.forEach((category) => {
                        ...
                        if (type == "filter") {
                            ...
                            <div
                                    class="p-1 lg:p-3 text-xs text-center lg:text-sm h-full flex items-center justify-center font-medium">
                                <a href="{{ url('ecomm_product/single-create/${category.id}') }}"
                                                   class="text-sky-700">محصول جدید</a>
                                            </div>
                                        @endif
                            </div>
