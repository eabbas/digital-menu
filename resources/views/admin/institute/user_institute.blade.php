...
@section('content')
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            ...
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center"> آموزشگاه های من</h2>
                ...
                <div
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]">
                        @php
                            $i = 1;
                        @endphp
                        @if($user->user_institutes)
                            @foreach ($user->user_institutes as $institute)
                                <div
                                ...
                                <div class="w-10 lg:w-full">
                                    <input type="checkbox" class="check" name="institutes[]" value="{{ $institute->id }}">
                                </div>
                            </div>
                            <div
                                            class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                                        ...
                                        <span class="block w-30 lg:w-full">{{ $institute->title }}</span>
                                        </div> --}}
                                        <div
                                            ...
                                            <a href="{{ route('institute.single', [$institute->id]) }}"
                                                class="block w-20 lg:w-full">{{ $institute->title }}</a>
                                        </div>
                                        <div
                                        class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
                                        {{-- <a href="{{ route('user.show', [Auth::user()->id]) }}"
                                            class="block w-30 lg:w-full">{{ Auth::user()->name }}
                                            {{ Auth::user()->family }}</a> --}}
                                        </div>
                                        <div
                                         class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 w-[500px] lg:w-full text-center col-span-2">
                                        <span class="block w-24 lg:w-full">{{ $institute->phone }}</span>
                                        </div>
                                 ...
                                 <div class="col-span-2">
                                    ...
                                    <ul class="text-sm mt-1 rounded-sm p-1 flex flex-cols-2 items-center justify-center">
                               ...
                               <div
                                class="p-1 lg:p-3 text-xs text-center lg:text-sm h-full flex items-center justify-center font-medium">
                                    <a href="{{ route('institute.myClasses' , [$institute]) }}"
                                        class="text-sky-700 text-nowrap">کلاس های من</a>
                                </div>
                                    </div>
