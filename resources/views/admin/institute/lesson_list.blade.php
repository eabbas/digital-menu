        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start border-b border-gray-200 p-4 gap-3">
                <h2 class="text-xl font-bold text-gray-800 text-center md:text-right">
                    لیست دروس آموزشگاه <span class="text-indigo-800">{{ $institute->title }}</span>
                </h2>
                <a href="{{ route('lesson.create', ['institute' => $institute->id]) }}"
                   class="inline-flex items-center gap-1 bg-indigo-800 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded-lg transition duration-200">
            ...
            <form action="{{ route('career.deleteAll') }}" method="POST" id="deleteAllForm" onsubmit="return confirm('آیا از حذف درس‌های انتخاب شده مطمئن هستید؟');">
                @csrf
                @method('DELETE')
                    ...
                    <table class="min-w-[700px] md:min-w-full w-full text-sm">
                        <thead class="bg-gray-100 text-gray-700 text-xs font-bold">
                            <tr>
                                ...
                                <th class="p-3 text-center">رشته</th>
                                <th class="p-3 text-center">عملیات</th>
</tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @forelse($institute->lessons as $index => $lesson)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                {{-- Checkbox --}}
                                <td class="p-3 text-center">
                                    <input type="checkbox" name="fields[]" value="{{ $lesson->id }}" class="check w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500">
                                </td>
                                {{-- Row number --}}
                                ...
                                <td class="p-3 text-center">
                                    @if($lesson->image)
                                        <img class="w-12 h-12 object-cover rounded-md mx-auto shadow-sm" src="{{ asset('storage/' . $lesson->image) }}" alt="{{ $lesson->title }}">
                                    @else
                                ...
                                {{-- Title --}}
                                <td class="p-3 text-center">
                                    <a href="{{ route('lesson.single', $lesson->id) }}" class="text-indigo-800 hover:text-indigo-800 font-medium">
                                        {{ $lesson->title }}
                                    </a>
                                </td>
                                {{-- Field (Career) --}}
                                ...
                                <td class="p-3 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('lesson.single', $lesson->id) }}" class="text-white bg-sky-500 hover:bg-sky-600 p-1.5 rounded-md transition" title="مشاهده">
...
80-80s-35.8-80-80-80c-.7 0-1.3 0-2 0c1.3 5.1 2 10.5 2 16c0 35.3-28.7 64-64 64c-5.5 0-10.9-.7-16-2c0 .7 0 1.3 0 2c0 44.2 35.8 80 80 80zm0-208a128 128 0 1 1 0 256 128 128 0 1 1 0-256z"/>
                                        ...
                                        <a href="{{ route('lesson.edit', $lesson->id) }}" class="text-white bg-green-500 hover:bg-green-600 p-1.5 rounded-md transition" title="ویرایش">
                                        ...
                                        <a href="{{ route('lesson.delete', $lesson->id) }}" onclick="return confirm('آیا از حذف درس «{{ $lesson->title }}» مطمئن هستید؟')" class="text-white bg-red-500 hover:bg-red-600 p-1.5 rounded-md transition" title="حذف">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 448 512">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-8 text-gray-500">
                                    ...
                                    <span>هیچ درسی برای این آموزشگاه یافت نشد.</span>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
