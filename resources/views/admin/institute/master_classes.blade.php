...
@section('content')
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            ...
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center">لیست کلاس ها</h2>
            ...
            <form class="flex flex-col gap-5" action="{{ route('career.deleteAll') }}" method="post">
                ...
                <div class="w-11/12 mx-auto flex flex-row justify-between items-center">
                    ...
                    <div class="flex justify-end flex-row gap-2 w-10/12 mx-auto">
                        <a href="{{ route('classAttachment.index') }}"
                            class="px-5 py-1 rounded-sm bg-blue-500 hover:bg-blue-600 text-white text-xs lg:text-base">لیست 
                            پیوست ها </a>
                            <a href="{{ route('classAttachment.create') }}"
                ...
                <div class="w-11/12 mx-auto shadow-md rounded mb-5 overflow-x-auto [&::-webkit-scrollbar]:hidden lg:overflow-visible">
                    <div class="w-full flex flex-row lg:grid lg:grid-cols-13 items-center divide-x divide-[#f1f1f4] sticky -top-5">
                        ...
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100 col-span-2">
                            <span class="block w-30 lg:w-full">درس</span>
                        </div>
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100 col-span-2">
                            <span class="block w-20 lg:w-full">استاد</span>
                        </div>
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]">
                        ...
                        @if($master->lesson_classes)
                            @foreach ($master->lesson_classes as $class)
                            @foreach ($master->institutes as $institute)
                            {{-- @dd($institute->id); --}}
                            @if ($class->lesson->field->institute->id == $institute->id)
                                
                            <div class="w-full flex flex-row lg:grid lg:grid-cols-13 items-center divide-x divide-[#f1f1f4]">
                                ...
                                <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                                    <div class="w-10 lg:w-full">
                                        <input type="checkbox" class="check" name="classes[]" value="{{ $class->id }}">
                                    </div>
                                </div>
                                ...
                                <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
                                    <a href="{{ route('class.single', [$class->id]) }}"
                                        class="block w-20 lg:w-full">{{ $class->title }}</a>
                                    </div>
                                    <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
                                        <a href="{{ route('lesson.single', [$class->lesson->id]) }}"
                                            class="block w-20 lg:w-full">{{ $class->lesson->title }}</a>
                                        </div>
                                        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 w-[500px] lg:w-full text-center col-span-2">
                                            @if($class->master)  <span class="block w-24 lg:w-full">{{$class->master->name}} {{$class->master->family}}</span> 
                                            @else <span class="block w-24 lg:w-full text-gray-400">بدون استاد</span>  @endif
                                        </div>
                                        
                                        <div class="col-span-4">
                                            <div
                                            class="w-[300px] lg:w-full grid grid-cols-3 divide-x divide-[#f1f1f4] items-center ">
                                            
                                            <ul class="text-sm mt-1 rounded-sm p-1 grid grid-cols-3">
                                                <li class="flex justify-center">
                                                    <a href="{{ route('class.single', [$class]) }}"
                                ...
                                <li class="flex justify-center">
                                    <a href="{{ route('class.delete', [$class]) }}"
                    ...
                    <a href="{{ route('class.attachments', [$class]) }}"
                    class="text-sky-700">پیوست ها</a>
                </div>
                <div
                class="p-1 lg:p-3 text-xs text-center lg:text-sm h-full flex items-center justify-center font-medium text-nowrap">
                <a href="{{ route('classAttachment.create', [$class]) }}"
                class="text-sky-700">افزودن پیوست</a>
            </div>
        </div>
    </div>
</div>
