...
@section('content')
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            ...
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center">لیست
                محصولات {{ Auth::user()->name.' '.Auth::user()->family }}</h2>

            <form class="flex flex-col gap-5" action="{{ route('ecomm_product.deleteAll') }}" method="post">
                @csrf
                ...
                <div
                    ...
                    <div
                        ...
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100 col-span-2">
                            <span class="block w-40 lg:w-full">عنوان</span>
                        </div>
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100">
                            <span class="block w-40 lg:w-full">قیمت</span>
                        </div>
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100">
                            <span class="block w-40 lg:w-full">تخفیف</span>
                        </div>
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100">
                            <span class="block w-20 lg:w-full">دسته ها</span>
                        </div>
                        <div class="px-1 lg:px-6 py-3 text-center text-xs font-medium text-gray-600 bg-gray-100 col-span-2">
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]">
                        ...
                        @foreach ($user->ecomm_categories as $ecomm_category)
                            @foreach($ecomm_category->ecomm_products as $product)
                                <div
                                    ...
                                    <div
                                        ...
                                        <div class="w-10 lg:w-full">
                                            <input type="checkbox" class="check" name="products[]"
                                                   value="{{ $product->id }}">
                                        </div>
                                    </div>
                                    <div
                                    ...
                                    <div
                                        ...
                                        <div class="w-20 lg:w-full">
                                           <a href="{{ route('ecomm_product.show', [$product->id]) }}">
                                               <img class="max-w-[50px] max-h-[50px] mx-auto size-12 object-cover rounded-md" src="{{ asset('storage/' . $product->image_path) }}">
                                           </a>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <a href="{{ route('ecomm_product.show', [$product->id]) }}"
                                           class="block w-40 lg:w-full">{{ $product->title }}</a>
                                    </div>
                                    <div
                                            class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                                        <span
                                           class="block w-40 lg:w-full">{{ number_format($product->price) }}</span>
                                    </div>
                                    <div
                                            class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                                        <span
                                           class="block w-40 lg:w-full">{{ number_format($product->discount) }}</span>
                                    </div>
                                    <div
                                            class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                                       @foreach($product->ecomm_categories as $category)
                                           <span class="w-20 lg:w-full block">{{ $category->title }}</span><br>
                                       @endforeach
                                    </div>
                                    <div class="col-span-2">

                                        <ul class="w-[120px] lg:w-full text-sm mt-1 rounded-sm p-1 grid grid-cols-3">
                                            ...
                                            <li class="flex justify-center">
                                                <a href="{{ route('ecomm_product.edit', [$product->id]) }}"
                                            ...
                                            <li class="flex justify-center">
                                                <a href="{{ route('ecomm_product.delete', [$product->id]) }}"
