@extends('admin.app.panel')
    ...
    <div class="bg-white rounded-lg">
      ...
      @foreach($siteLinks as $siteLinks)
                        @if($siteLinks)
                        <div class="grid grid-cols-8 items-center divide-x divide-[#f1f1f4]">
                            <div class="p-3 text-sm h-full flex items-center justify-center text-gray-900">{{ $siteLinks->title}}</div>
                            </div>
                        <div class="grid grid-cols-8 items-center divide-x divide-[#f1f1f4]">
                            <div class="p-3 text-sm h-full flex items-center justify-center text-gray-900">{{ $siteLinks->address}}</div>
                            </div>
                            <div class="p-3 text-sm h-full flex items-center justify-center text-gray-900">
                                <img class="max-w-[50px] max-h-[50px] mx-auto size-12 object-cover" src="<?= asset("storage/" . $siteLinks->icon_path) ?>">
                            </div>
                            <div class="w-full grid grid-cols-2 col-span-3 h-full divide-x divide-[#f1f1f4]">
                                <div class="p-3 text-sm h-full flex items-center justify-center font-medium">
                                    <a href="{{ route('siteLink.edit', [$siteLinks])}}" class="ml-4 text-sky-700">ویرایش</a>
                                </div>
                            </div>
                            <div class="w-full grid grid-cols-2 col-span-3 h-full divide-x divide-[#f1f1f4]">
                                <div class="p-3 text-sm h-full flex items-center justify-center font-medium">
                                    <a href="{{ route('siteLink.delete', [$siteLinks])}}" class="ml-4 text-sky-700">حذف</a>
                                </div>
                            </div>
                        </div>
                        @else
