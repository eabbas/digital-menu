            ...
            <form action="{{ route('covers.update') }}" method="post" enctype='multipart/form-data' class="w-11/12 lg:w-3/4 mx-auto p-5 rounded-lg border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-5 lg:gap-10">
                    ...
                    <div class="w-full flex flex-col">
                        ...
                        <input type="text" name="title"  value="{{$cover -> title}}" class="w-full px-2 py-1 lg:px-2 outline-none border-b" required>
                    </div>
                    <div class="w-full flex flex-col">
                        <label class="text-sm md:text-base" for="subTitle">  زیرعنوان  :</label>
                        <input type="text" name="subTitle"  value="{{$cover -> subTitle}}" class="w-full px-2 py-1 lg:px-2 outline-none border-b" required>
                    </div>
                    <div class="w-full flex flex-col">
                        <label class="text-sm md:text-base" for="description"> توضیحات:</label>
                        <input type="text" name="description"  value="{{$cover -> description}}" class="w-full px-2 py-1 lg:px-2 outline-none border-b" required>
                    </div>
