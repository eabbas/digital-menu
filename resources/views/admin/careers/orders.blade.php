...
window.Echo.channel('orderNotification{{ $career->id }}')
    .listen('orders', function(e) {
        console.log('سفارش جدید:', e)
        
        let order = e.order_data
        let grid = document.querySelector('#parentOrdersBlock')
        if (!grid) return
        ...
        if (e.is_cancelled) {
            ...
            let careerCard = document.getElementById('career-card-' + order.career_id)
            if (careerCard) {
                let countElement = careerCard.querySelector('.order-count')
                if (countElement) {
                    countElement.textContent = e.cart_count + ' سفارش'
                }
                
                if (e.cart_count === 0) {
                    let ordersBtn = careerCard.querySelector('.orders-btn')
                    if (ordersBtn) {
        ...
        card.className = 'w-full p-3 bg-white rounded-md flex flex-col shadow-[0px_0px_2px_1px_#e7e7ef] orders';
        ...
        card.innerHTML = `
            <div class="w-full flex flex-row items-center justify-between pb-3 border-b border-[#e4e5ea]">
                <div class="flex flex-row items-center gap-2">
                    <div class="size-8 bg-[#f4f4f6] rounded-full flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-6" viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M19 10H29"/>
                            <path d="M19 10V16"/>
                            <path d="M29 10V16"/>
                            <path d="M10 10V36"/>
            ...
            <div class="pt-3 w-full flex flex-col gap-3.5">
                <div class="w-full flex flex-row items-center">
                    <div class="w-1/3 flex flex-col items-center justify-center gap-1.5 border-l-1 border-[#e4e5ea]">
                        <div class="flex flex-row items-center gap-0.5">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="var(--secondary-text-color)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M6 8H18L19 20H5L6 8Z"/>
                                <path d="M8.5 8V6.8C8.5 4.7 10 3.5 12 3.5C14 3.5 15.5 4.7 15.5 6.8V8"/>
                            </svg>
                            <span class="text-[10px] text-(--secondary-text-color)">تعداد آیتم</span>
                        </div>
                        <span class="text-xs font-bold text-(--primary-text-color) in-fa order-item-count">${order.carts ? order.carts.length : 0} آیتم</span>
                    </div>
                    <div class="w-1/3 flex flex-col items-center justify-center gap-1.5 border-l-1 border-[#e4e5ea]">
                        <div class="flex flex-row items-center gap-1.5">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-4 fill-(--secondary-text-color)" viewBox="0 0 448 512">
                                <path d="M112 0c8.8 0 16 7.2 16 16V64H320V16c0-8.8 7.2-16 16-16s16 7.2 16 16V64h32c35.3 0 64 28.7 64 64v32 32V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V192 160 128C0 92.7 28.7 64 64 64H96V16c0-8.8 7.2-16 16-16zM416 192H312v72H416V192zm0 104H312v80H416V296zm0 112H312v72h72c17.7 0 32-14.3 32-32V408zM280 376V296H168v80H280zM168 408v72H280V408H168zm-32-32V296H32v80H136zM32 408v40c0 17.7 14.3 32 32 32h72V408H32zm0-144H136V192H32v72zm136 0H280V192H168v72zM384 96H64c-17.7 0-32 14.3-32 32v32H416V128c0-17.7-14.3-32-32-32z"/>
                            </svg>
                            <span class="text-[10px] text-(--secondary-text-color)">تاریخ</span>
                        </div>
                    ...
                    <div class="w-1/3 flex flex-col items-center justify-center gap-1.5">
                        <div class="flex flex-row items-center gap-1.5">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="var(--secondary-text-color)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                                <circle cx="12" cy="12" r="6.8"/>
                                <path d="M12 8.5V12"/>
                                <path d="M12 12L14.8 13.8"/>
                            </svg>
                ...
                <div class="w-full flex flex-col gap-1">
                    <div class="w-full flex flex-row items-center gap-2">
                        ...
                        <button class="w-[25%] flex flex-row gap-1 items-center justify-center py-1.5 rounded-md border-1 border-[#e4e5ea] text-(--secondary-text-color) text-xs cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 24 24" fill="none" stroke="var(--secondary-text-color)" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
                                <rect x="8.2" y="4" width="7.6" height="3.8" rx="0.4"/>
                                <rect x="4.8" y="8" width="14.4" height="6.8" rx="0.8"/>
                                <rect x="8.2" y="13.2" width="7.6" height="6.8" rx="0.4"/>
                                <circle cx="17" cy="11.2" r="0.5" fill="currentColor" stroke="none"/>
                        ...
                        <button class="block w-[20%] flex flex-row gap-1 items-center justify-center py-1.5 rounded-md border-1 border-rose-500 text-rose-500 text-xs cursor-pointer" onclick='deleteOrder(this, "${order.id}")'>
                            حذف
                    ...
                    <div class="w-full flex justify-center">
                        <button class="min-w-full max-w-full flex items-center justify-center gap-0.5 bg-(--color-green) py-1 rounded-lg cursor-pointer order-accept-btn" onclick='accept("${order.id}", this, "${order.status.id}")'>
                            <span class="text-white text-sm font-bold">${order.status.admin_title}</span>
                            <svg class="size-6 rounded-lg" viewBox="0 0 36 36" fill="white">
                                <path class="clr-i-outline clr-i-outline-path-1" d="M18,6A12,12,0,1,0,30,18,12,12,0,0,0,18,6Zm0,22A10,10,0,1,1,28,18,10,10,0,0,1,18,28Z"></path>
                                <path d="M16.34,23.74l-5-5a1,1,0,0,1,1.41-1.41l3.59,3.59,6.78-6.78a1,1,0,0,1,1.41,1.41Z"></path>
        ...
        if (msg) {
            msg.innerHTML = `
                <div class="relative p-4">
                    ...
                    <div class="text-center text-sm text-gray-700">✅ سفارش جدید دریافت شد!</div>
                </div>
            `;
