...
@section('content')
    ...
    @if (isset($custom_product))
        <div class="fixed w-full h-dvh z-999 top-0 right-0 bg-black/50 invisible opacity-0 transition-all duration-300 form"
            ...
            <div class="w-full lg:w-[calc(100%-265px)] float-end flex justify-center items-center h-dvh relative"
                ...
                <form action="{{ route('cp.update') }}" method="post" enctype="multipart/form-data" class="bg-white w-11/12 lg:w-1/2 p-5 rounded-lg relative">
                        <div id="editCPloading"
                            class="w-full absolute h-full top-0 right-0 bg-white items-center justify-center hidden rounded-lg">
                        </div>
                        @csrf
                        <input type="hidden" name="id" id="custom_product_id">
                        <input type="hidden" name="career_id" id="careerId" value="{{ $custom_product->career->id }}">
                        <div class="mb-4">
    ...
    @endforeach
    @foreach ($allCustomProduct as $customProduct)
        <input type="hidden" id="careerIdCustomProduct" value="{{ $customProduct->career->id }}">
    @endforeach
    </div>
    ...
    <script>
        ...
        let careerIdCustomProduct = document.getElementById('careerIdCustomProduct')
        ...
        function storeCP(ev) {
            ...
            formData.append('career_id' ,careerIdCustomProduct.value)
