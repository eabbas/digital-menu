...
<div class="w-full min-h-screen pb-10 pt-6 bg-gradient-to-b from-blue-50 to-white">
                    ...
                    <div class="w-11/12 mx-auto shadow-md rounded mb-5 overflow-x-auto [&::-webkit-scrollbar]:hidden lg:overflow-visible">
                        ...
                        <div class="bg-white divide-y divide-[#f1f1f4]" id="custom_product_section">
                            ...
                            @foreach ($allCustomProduct as $index => $custom_product)
                                ...
                                <div class="w-full flex flex-row lg:grid lg:grid-cols-12 items-center divide-x divide-[#f1f1f4] newParameters"  data-cp-id="{{ $custom_product->id }}">
        ...
        <div class="w-[calc(100%-265px)] float-end flex justify-center items-center h-dvh relative" id="closeCreateCform">
            ...
            <form action="{{ route('custmCategory.store') }}" method="post" enctype="multipart/form-data" class="bg-white w-1/2 p-5 rounded-lg">
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="max_item_amount" id="max_item_amount_customCategory" required min="1"
    ...
    <!-- فرم ویرایش محصول (این فرم باید بیرون از if باشد) -->
    <div class="fixed w-full h-dvh z-999 top-0 right-0 bg-black/50 invisible opacity-0 transition-all duration-300 form" id="editCPform">
        <div class="w-[calc(100%-265px)] float-end flex justify-center items-center h-dvh relative bg-white" id="closeEditCform">
            ...
            <form method="post" class="w-11/12 lg:w-3/4 mx-auto py-5 rounded-lg">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3 lg:gap-4">
                    <div class="w-full flex flex-col">
                        <fieldset class="text-sm md:text-base border border-gray-400 rounded-[20px] sm:py-1 pr-3">
                            ...
                            <input type="text" name="editTitleCP" id="editTitleCP"
                    ...
                    <div class="w-full flex flex-col">
                        <fieldset class="text-sm md:text-base border border-gray-400 rounded-[20px] sm:py-1 pr-3">
                            <legend class="p-1 w-20 sm:bg-blue-400 sm:text-white rounded-full flex flex-row justify-center text-sm">
                                توضیحات</legend>
                            <input type="text" name="description" id="editDescriptionCP"
                                class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500">
                        </fieldset>
                    </div>
                    ...
                    <div class="w-full flex flex-col">
                        <fieldset class="text-sm md:text-base border border-gray-400 rounded-[20px] sm:py-1 pr-3">
                            ...
                            <input type="number" name="material_limit" id="material_limit_CP"
                                class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500">
                        </fieldset>
                    </div>
                </div>
                <div class="text-center md:px-12 mt-5 lg:mt-10">
                    <button type="button" onclick="updateCP(event)"
                        class="w-5/12 max-sm:bg-blue-500 max-sm:text-white px-5 py-2 lg:px-10 lg:py-3 rounded-[8px] transition-all duration-250 bg-blue-400 text-white hover:bg-blue-600 hover:border-gray-400 hover:text-white text-gray-500 cursor-pointer">
                        ویرایش
                    </button>
                </div>
            </form>
...
<script>
    ...
    function updateCP(ev){
        ...
        // دریافت career_id از select موجود در فرم ایجاد
        let careerSelect = document.querySelector('select[name="career_id"]');
        let careerId = careerSelect ? careerSelect.value : null;
            ...
            success: function(data){
                newParameters.forEach((element) => {
                    if(element.getAttribute('data-cp-id') == data.id) {
                        // به روزرسانی مقادیر در جدول
                        // ستون نام محصول (index 2)
                        element.children[2].children[0].innerText = data.title;
                        // ستون حد مواد (index 3)
    ...
    function storeCP(ev){
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {
                'career_id': document.querySelector('select[name="career_id"]').value,
                'title': title.value,
            ...
            success: function(data){
                ...
                let element = `
                    <div class="w-full flex flex-row lg:grid lg:grid-cols-12 items-center divide-x divide-[#f1f1f4] newParameters" data-cp-id="${data.id}">
                        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center">
                            <span class="block w-10 lg:w-full">${i}</span>
                        </div>
                        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900">
                        ...
                        <div class="col-span-6">
                            <div class="w-[320px] lg:w-full grid grid-cols-5 divide-x divide-[#f1f1f4] items-center">
                                <ul class="text-sm mt-1 rounded-sm p-1 grid grid-cols-3 gap-4">
                                    ...
                                    <li class="flex justify-center" onclick='editCP(${data.id})'>
                                        <div class="w-fit flex flex-row items-center justify-center bg-green-500 hover:bg-green-600 p-1 rounded-sm" title="ویرایش">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 512 512">
                                                <path fill="white" d="M441 58.9L453.1 71c9.4 9.4 9.4 24.6 0 33.9L424 134.1 377.9 88 407 58.9c9.4-9.4 24.6-9.4 33.9 0zM209.8 256.2L344 121.9 390.1 168 255.8 302.2c-2.9 2.9-6.5 5-10.4 6.1l-58.5 16.7 16.7-58.5c1.1-3.9 3.2-7.5 6.1-10.4zM373.1 25L175.8 222.2c-8.7 8.7-15 19.4-18.3 31.1l-28.6 100c-2.4 8.4-.1 17.4 6.1 23.6s15.2 8.5 23.6 6.1l100-28.6c11.8-3.4 22.5-9.7 31.1-18.3L487 138.9c28.1-28.1 28.1-73.7 0-101.8L474.9 25C446.8-3.1 401.2-3.1 373.1 25zM88 64C39.4 64 0 103.4 0 152V424c0 48.6 39.4 88 88 88H360c48.6 0 88-39.4 88-88V312c0-13.3-10.7-24-24-24s-24 10.7-24 24V424c0 22.1-17.9 40-40 40H88c-22.1 0-40-17.9-40-40V152c0-22.1 17.9-40 40-40H200c13.3 0 24-10.7 24-24s-10.7-24-24-24H88z"/>
                                            </svg>
    ...
    function cpvStore(ev){
        ...
        $.ajax({
            ...
            data: {
                ...
                'custom_product_id': custom_product_id_variant.value,
            },
            success: function(data){
