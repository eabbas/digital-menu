...
@section('content')
    <div class="flex items-start justify-center lg:mb-10 mb-5">
        <form action="{{ route('introPro.updatePage', [$product->id]) }}" method="post" enctype="multipart/form-data" class="bg-white rounded-2xl p-3 w-full lg:w-3/4">
            @csrf
            <div class="text-center mb-8">
                <h3 class="lg:text-lg font-bold text-gray-800">{{ $product->title }}</h3>
            </div>
            <div class="text-center mb-4">
                <div class="w-full grid grid-cols-1 lg:grid-cols-2 gap-3 my-4">
                    <div
                        ...
                        <div
                            ...
                            <input
                                    ...
                                    placeholder="عنوان محصول" required value="{{ $product->title }}">
                        </div>
                    </div>
                    <div
                            class="w-full flex flex-col gap-3 itmes-center max-md:flex-col max-md:gap-1">
                        ...
                        <div
                            ...
                            <select
                                ...
                                @foreach($product->page->introCats as $category)
                                    <option value="{{ $category->id }}" @if(in_array($category->id, $product->categories->pluck('id')->all())) selected @endif>{{ $category->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="w-full flex flex-row gap-3 itmes-center max-md:gap-1">
                        ...
                        <input type="checkbox" name='show_in_home' id="show_in_home" @if($product->show_in_home == 1) checked @endif>
                    </div>
                    <div
                            class="w-full flex flex-col gap-3 itmes-center max-md:flex-col max-md:gap-1">
                    ...
                    <div
                        ...
                        @if($product->attributes)
                            @foreach(json_decode($product->attributes) as $attribute)
                                <div class="w-full grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 border-1 border-gray-300 rounded-lg relative feature-row">
                                    <span class="absolute -top-2 left-[-8px] px-2 py-1 bg-white rounded-full text-sm cursor-pointer shadow delete-btn closeBTN">
                                        ❌
                                    </span>
                                    <input
                                        ...
                                        type="text"
                                        placeholder="نام ویژگی" value="{{ $attribute->key }}">
                                    <input
                                        ...
                                        type="text"
                                        placeholder="مقدار ویژگی" value="{{ $attribute->value }}">
                                </div>
                                @php $i++ @endphp
                            @endforeach
                        @endif
