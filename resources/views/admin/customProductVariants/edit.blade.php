...
<section class="2xl:container mx-auto">
    <div class="w-full mx-auto pb-5">
        <fieldset class="border-2 rounded-[10px] border-gray-400 shadow">
            ...
            @csrf
            @foreach ($customProduct->custom_product_variants as $variant)
            <input type="hidden" name="id" value="{{$variant -> id}}">
             <input type="hidden" name="id" value="{{$customProduct -> id}}">
                <input type="hidden" name="career_id" value="{{ $customProduct->career }}">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <div class="w-full flex flex-col">
                        ...
                        <legend class="p-1 w-20 sm:bg-blue-400 sm:text-white rounded-full flex flex-row justify-center text-sm">
                            توضیحات</legend>
                            <input type="text" name="description" value="{{$variant -> description}}"
                                class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500">
                        </fieldset>
                    </div>
                    <div class="w-full flex flex-col">
                        <fieldset class="text-sm md:text-base border border-gray-400 rounded-[20px] sm:py-1 pr-3"
                            ...
                            <legend class="p-1 w-30 sm:bg-blue-400 sm:text-white rounded-full flex flex-row justify-center text-sm">
                                ...
                                <input type="text" name="min_amount_unit" value="{{$variant -> min_amount_unit}}"
                                class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500">
                            </fieldset>
                        </div>
                        <div class="w-full flex flex-col">
                            ...
                            <legend class="p-1 w-30 sm:bg-blue-400 sm:text-white rounded-full flex flex-row justify-center text-sm">
                                ...
                                <input type="text" name="duration" value="{{$variant -> duration}}"
                                class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500">
                            </fieldset>
                        </div>
                        <div class="w-full flex flex-col">
                            ...
                            <input type="file" name="image" class="w-full px-2 py-1 lg:px-2 outline-none text-gray-500" value={{ $variant->image }}>
                        </fieldset>
                    </div>
                </div>
                
                <input type="hidden" name="custom_product_id" value="{{ $customProduct->id }}">
