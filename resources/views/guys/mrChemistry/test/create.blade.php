...
@section('content')
    <div class="absolute top-0 opacity-0 invisible right-1/2 translate-x-1/2 w-2/3 lg:w-1/3 bg-white rounded-lg shadow-md transition-all duration-500"
        ...
        <h1 class="text-2xl font-bold text-gray-800 text-center mb-5">ایجاد آزمون </h1>

    <form action="{{ route('mtest.store') }}" method="post" enctype='multipart/form-data'>
        @csrf
                        ...
                        <div class="w-full flex flex-col gap-3 itmes-center max-md:flex-col max-md:gap-1">
                            ...
                            <div
                                ...
                                <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="text"
                                       name='title' placeholder="عنوان آزمون" id="title">
                            </div>
                        </div>
                        <div class="w-full flex flex-col gap-3 itmes-center max-md:flex-col max-md:gap-1">
                            ...
                            <div
                                ...
                                <select name="educational_base_id"
                                    ...
                                    @foreach($educationalBase as $base)
                                        <option value="{{ $base->id }}" @if($base->id == 1) selected @endif>{{ $base->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="w-full flex flex-col gap-3 itmes-center max-md:flex-col max-md:gap-1">
                            ...
                            <div
                                ...
                                <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="number"
                                       name='duration' placeholder="120" id="duration">
                            </div>
                        </div>
